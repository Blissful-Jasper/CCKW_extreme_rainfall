...
def process_single_level(task: dict) -> str:
    ...
    if _WORKER_CATALOG is None:
        _WORKER_CATALOG = intake.open_catalog(task["catalog_url"])
    ...
    if result.get("failed", 0):
        raise RuntimeError(
            ...
            f"level {int(task['level']):03d} failed"
        )
    return (
        f"{task['var_name']} {task['exp_key'].upper()} "
...
def process_layers(args: argparse.Namespace, target_lat: np.ndarray, target_lon: np.ndarray) -> None:
    ...
    with dask_runtime(args):
        for var_name, experiments in work.items():
            ...
            failed = [
                f"{var_name} {exp_name}"
                for exp_name, result in results.get(var_name, {}).items()
                if isinstance(result, dict) and (result.get("failed", 0) or "error" in result)
            ]
            if failed:
                raise RuntimeError(f"Failed level processing: {', '.join(failed)}")
